...
def prob_mask_like(shape, prob, device):
    if prob == 1:
        ...
        return torch.zeros(
            shape, device=device).float().uniform_(
            0, 1) < prob
...
class RelativePositionBias(nn.Module):
    ...
    def _relative_position_bucket(
        ...
        val_if_large = torch.min(
            val_if_large, torch.full_like(
                val_if_large, num_buckets - 1))
    ...
    def forward(self, n, device):
        ...
        rel_pos = rearrange(
            k_pos, 'j -> 1 j') - rearrange(q_pos, 'i -> i 1')
...
class EinopsToAndFrom(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        x = self.fn(x, **kwargs)
        x = rearrange(
            x, f'{self.to_einops} -> {self.from_einops}', **
            reconstitute_kwargs)
        return x
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if exists(focus_present_mask) and not (
                ~focus_present_mask).all():
            attend_all_mask = torch.ones(
                (n, n), device=device, dtype=torch.bool)
            attend_self_mask = torch.eye(
...
class Unet3D(nn.Module):
    def __init__(
        ...
        def temporal_attn(dim): return EinopsToAndFrom(
            'b c f h w', 'b (h w) f c',
            Attention(
                dim, heads=attn_heads, dim_head=attn_dim_head,
    ...
    def forward(
        ...
        if self.has_cond:
            ...
            mask = prob_mask_like(
                (batch,),
                null_cond_prob, device=device)
            cond = torch.where(
                rearrange(mask, 'b -> b 1'),
...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        def register_buffer(
            name, val): return self.register_buffer(
        ...
        register_buffer('posterior_log_variance_clipped', torch.log(
    ...
    def q_mean_variance(self, x_start, t):
        mean = extract(self.sqrt_alphas_cumprod,
    ...
    def p_sample(self, x, t, cond=None, cond_scale=1.,
                 clip_denoised=True):
    ...
    def p_sample_loop(self, shape, cond=None, cond_scale=1.):
        ...
        for i in tqdm(reversed(range(0, self.num_timesteps)),
            ...
            img = self.p_sample(
                img, torch.full(
                    (b,),
                    i, device=device, dtype=torch.long),
    ...
    def interpolate(self, x1, x2, t=None, lam=0.5):
        ...
        xt1, xt2 = map(lambda x: self.q_sample(
            x, t=t_batched), (x1, x2))

        img = (1 - lam) * xt1 + lam * xt2
        for i in tqdm(
                reversed(range(0, t)),
                desc='interpolation sample time step', total=t):
            img = self.p_sample(
                img, torch.full(
                    (b,),
                    i, device=device, dtype=torch.long))
    ...
    def p_losses(self, x_start, t, cond=None, noise=None, **kwargs):
        ...
        if is_list_str(cond):
            cond = bert_embed(
                tokenize(cond),
                return_cls_repr=self.text_use_bert_cls)
            cond = cond.to(device)
    ...
    def forward(self, x, *args, **kwargs):
        ...
        check_shape(x, 'b f c h w -> b c f h w', c=self.channels,
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        self.dl = cycle(
            data.DataLoader(
                self.ds, batch_size=train_batch_size, shuffle=True,
                pin_memory=True))
    ...
    def save(self, milestone):
        ...
        torch.save(
            data, str(self.results_folder / f'model-{milestone}.pt'))
        ...
        for _ in tqdm(range(self.train_num_steps)):
            for i in range(self.gradient_accumulate_every):
                ...
                with autocast(enabled=self.amp):
                    ...
                    self.scaler.scale(
                        loss /
                        self.gradient_accumulate_every).backward()
            ...
            if self.step != 0 and self.step % self.save_and_sample_every == 0:
                ...
                one_gif = rearrange(
                    all_videos_list, '(i j) c f h w -> c f (i h) (j w)',
                    i=self.num_sample_rows)
                video_path = str(self.results_folder /
                                 str(f'{milestone}.gif'))
